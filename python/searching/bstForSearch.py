...
class BinarySearchTree:
    ...
    def breadthFirstTraversal(self):
        ...
        searchQueue = []
        searchQueue.append(currNode)

        while currNode != None:
            if currNode.left != None:
                searchQueue.append(currNode.left)
            if currNode.right != None:
                searchQueue.append(currNode.right)
            traversalOrderResult.append(searchQueue.pop(0).value)
    ...
    def recursiveBFTraversal(self, searchQueue, traversalOrderResult):
        if len(searchQueue) == 0:
            return traversalOrderResult

        currNode = searchQueue.pop(0)
        traversalOrderResult.append(currNode.value)
        if currNode.left != None:
            searchQueue.append(currNode.left)
        if currNode.right != None:
            searchQueue.append(currNode.right)

        return self.recursiveBFTraversal(searchQueue, traversalOrderResult)

    def inorderDFS(self, node, arr):
        if node.left != None:
            self.inorderDFS(node.left, arr)
        arr.append(node.value)
        if node.right != None:
            self.inorderDFS(node.right, arr)
        return arr

    def preorderDFS(self, node, arr):
        arr.append(node.value)
        if node.left != None:
            self.preorderDFS(node.left, arr)
        if node.right != None:
            self.preorderDFS(node.right, arr)
        return arr

    def postorderDFS(self, node, arr):
        if node.left != None:
            self.postorderDFS(node.left, arr)
        if node.right != None:
            self.postorderDFS(node.right, arr)
        arr.append(node.value)
        return arr
